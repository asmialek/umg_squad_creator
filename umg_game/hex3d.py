...
class HexMap(object):
    ...
    def reset(self):
        ...
        self.scale = 10
        self.recenter(self.start_center)
        self.change = True
    ...
    def update(self):
        ...
        if pg.mouse.get_pressed()[0] and pg.key.get_mods() & pg.KMOD_LSHIFT:
            self.recenter((self.center[0]+rel[0],self.center[1]+rel[1]))
        if self.change:
    ...
    def get_event(self, event):
        if event.type == pg.KEYDOWN:
            if event.key == pg.K_LEFT:
                ...
                self.recenter((self.center[0]+10,self.center[1]))
...
def make_tiles(rot, scale, squash):
    ...
    for biome,_ in TERRAIN:
        ...
        # surf.fill((0,90,20))
        # if not once:
        #     pg.draw.polygon(surf, (250, 200, 200), top)
        #     once = True
